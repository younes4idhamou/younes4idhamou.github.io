...
class NpDataProvider {
    ...
    public function getStyleCss() {
        ...
        if ($css_parts) {
            ...
            $modal_dialogs_json_str = NpMeta::get('modal_dialogs_css_used_ids');
            $modal_dialogs_css_used_ids = $modal_dialogs_json_str ? json_decode($modal_dialogs_json_str, true) : array();
            ...
            foreach ($css_parts as $part) {
                if ($part['type'] !== 'color'
                    ...
                    || !empty($modal_dialogs_css_used_ids[$part['id']])
     ...
     */
    public function setStyleCss($styles, $publish_html, $publishHeaderFooter = '', $publishCookiesSection = '', $modalDialogs = '') {
        ...
        if ($modalDialogs) {
            $used_ids = array();
            foreach ($parts as &$part) {
                if (isset($part['id']) && strpos($modalDialogs, preg_replace('#^color_#', '', $part['id'])) !== false) {
                    $used_ids[$part['id']] = true;
                }
            }
            NpMeta::update('modal_dialogs_css_used_ids', json_encode($used_ids));
        }
